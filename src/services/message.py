...
class MessageService:
    ...
    async def create_test_data(self):
        ...
        for i in range(5):
            ...
            chat = await self.chat_service.create_personal_chat(
                users_in_chat[0].id, users_in_chat[1].id
            )
            chats.append(chat)

        for i in range(5, 10):
            chat = await self.chat_service.create_group_chat(
                name=f"Chat {i}", creator_id=choice(users).id
            )
            chats.append(chat)
